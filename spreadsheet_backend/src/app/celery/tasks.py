...
@celery_app.task
def update_cell_task(
    spreadsheet_id, row_index, col_index, value, formula=None, references=None
):
    session = Session()
    db_client = DBClient(session)
    try:
        response = db_client.update_cell(
            spreadsheet_id, row_index, col_index, value, formula
        )
        if response["success"]:
            logger.info(
                f"Updated cell ({row_index}, {col_index}) for spreadsheet {spreadsheet_id} in DB"
            )
        else:
            logger.error(
                f"Failed to update cell ({row_index}, {col_index}) for spreadsheet {spreadsheet_id} in DB: {response['error_type']}"
            )
...
def export_spreadsheet_task(spreadsheet_id):
    ...
    try:
        ...
        if not spreadsheet["success"]:
            logger.error(
                f"Failed to retrieve spreadsheet {spreadsheet_id} for export: {spreadsheet['error_type']}"
            )
            return None
